...
public class DatabaseManager {
    ...
    private void connect() {
        try {
            ...
            e.printStackTrace(); // Replace with better error logging
        }
    }
     ...
     */
    public void saveToDatabase(FileEvent theEvent) {
        String sql = """
    ...
    /**
     * Queries events by extension.
     */
    public List<FileEvent> queryByExtension(String ext) {
        return queryByCondition("file_extension = ?", ext);
    }

    /**
     * Queries events by event type.
     */
    public List<FileEvent> queryByEventType(EventType type) {
        return queryByCondition("event_type = ?", type.toString());
    }

    /**
     * Queries events by directory path.
     */
    public List<FileEvent> queryByDirectory(String pathPrefix) {
        return queryByCondition("path LIKE ?", pathPrefix + "%");
    }

    /**
     * Queries events within a date range.
     */
    public List<FileEvent> queryByDateRange(LocalDateTime start, LocalDateTime end) {
        String sql = "SELECT * FROM file_events WHERE timestamp >= ? AND timestamp <= ?;";
        List<FileEvent> results = new ArrayList<>();
        try (PreparedStatement pstmt = myConnection.prepareStatement(sql)) {
     ...
     */
    private List<FileEvent> queryByCondition(String clause, String value) {
        List<FileEvent> results = new ArrayList<>();
     ...
     */
    private FileEvent parseResultSet(ResultSet rs) throws SQLException {
